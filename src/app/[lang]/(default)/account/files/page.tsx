...
export default async function Page({ params: { lang } }: { params: { lang: string } }) {
    ...
    const filesInformation = lang === 'en'
        ? "Your files are listed on an appointment basis."
        : "Dosyaların randevu bazında listelenmektedir.";
    // static language switch area finish


    return (
        <div>
            <div className="p-4 text-xs text-gray-800 rounded tracking-wide bg-gray-50 " role="alert">
                {(filesInformation)}
            </div>
            <div className="flex flex-col gap-4 mt-5">
                {myAppointments.map((appointment: AppointmentData, index: number) => (
                    <AppointmentItem key={index} lang={lang} appointment={appointment} token={token} />
                ))}
            </div>
        </div>
    );
