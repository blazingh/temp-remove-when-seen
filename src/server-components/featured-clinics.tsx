...
export function FeaturedClinicsListSkeleton() {
  return (
...
export default async function FeaturedClinicsList({ }: {}) {
  const randomSeed = Math.floor(Math.random() * 100);

  const data = await getFeaturedClinics({ randomSeed: randomSeed });
    ...
    >
      <CarouselContent>
        {data.rows?.map(
          (item) =>
            item.url !== "" && (
              <CarouselItem key={item.id} className="basis-[167px] mr-4">
                <FeaturedClinicCard
                  item={item}
                  href={{
