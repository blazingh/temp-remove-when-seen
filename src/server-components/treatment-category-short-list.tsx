...
export default async function TreatmentCategoryShortList({
  domain,
}: {
  domain: string;
}) {
  ...
  if (error1 || !categories || categories.data.length === 0) return "sdsd";

  const [error2, treatments] = await safeAwait(
    getStrapiData(
  ...
  if (error2 || !treatments || treatments.data.length === 0) return null;
...
export function TreatmentCategoryShortListSkeleton() {
  return (
